...
class PiczakCNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(
            80 * 3 * 1, 5000
        )  # Adjusted to match output size from conv layers
...
def detect_sound(model_path, audio_path, label_to_int, int_to_label):
    ...
    segments = torch.tensor(X_long.transpose(0, 3, 1, 2), dtype=torch.float32).to(
        device
...
audio_path = "ESC-10/0_dog/3-180256-A.ogg"
